...
const int int_bytes = int(std::ceil((std::log2(std::numeric_limits<int>::max()))/8));
...
void main()
{
	...
	SDL_Window * cam_window = SDL_CreateWindow("Camera Input",
        SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 32, 24, 0);
	...
	xt::static_shape<std::size_t, 4> pic_shape = { 1, capture.mWidth, capture.mHeight, int_bytes};
	...
	xt::xtensor<float, 4> filter = xt::random::rand<float>({4,4,3,3}, 0, 120);
	xt::xtensor<float, 4> d_filter = xt::zeros<float>({4,4,3,3});
	...
	while(!quit){
		...
		auto img = xt::adapt(pic, capture.mWidth * capture.mHeight*int_bytes, false, pic_shape);

		SDL_UpdateTexture(img_texture, NULL, static_cast<void*>(img.data()), capture.mWidth*int_bytes);
		...
		//auto noised_img_display = xt::transpose(noised_img, {0,3,2,1}); // to image shape

		SDL_UpdateTexture(noised_texture, NULL, static_cast<void*>(noised_img.data()), capture.mWidth*int_bytes);
		SDL_RenderCopy(noised_renderer, noised_texture, NULL, NULL);
		SDL_RenderPresent(noised_renderer);

		auto pada = pad(a);
		auto d_inp = xt::zeros<float>(pada.shape());

        auto res = conv2d_forward(pada, filter);
		//auto res_display = xt::transpose(res, {0,3,2,1}); // to image shape


		SDL_UpdateTexture(out_texture, NULL, static_cast<void*>(res.data()), capture.mWidth*int_bytes);
		SDL_RenderCopy(out_renderer, out_texture, NULL, NULL);
		SDL_RenderPresent(out_renderer);

		xt::xtensor<float, 4> res_cast = pad(res);
		xt::xtensor<float, 4> img_cast = pada;
		xt::xtensor<float, 4> err = res_cast - img_cast;
